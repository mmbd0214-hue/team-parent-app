...
async function loadParents(){cache.parents=await api("/api/admin/parents");$("parentTable").innerHTML=`<div class="tableWrap"><table><thead><tr><th>家長</th><th>電話</th><th>LINE User ID</th><th>綁定球員</th><th>操作</th></tr></thead><tbody>${cache.parents.map(p=>`<tr><td><strong>${p.display_name}</strong></td><td>${p.phone||"-"}</td><td><code>${p.line_user_id}</code></td><td>${(p.players||[]).map(x=>`<span class="tag">${x.name}/${x.team}</span>`).join("")||'<span class="tag amber">尚未綁定</span>'}</td><td><div class="rowActions"><button onclick="openBind(${p.id},null)">綁定球員</button>${(p.players||[]).map(x=>`<button onclick="unbind(${p.id},${x.id})">解除 ${x.name}</button>`).join("")}</div></td></tr>`).join("")}<button onclick="deleteParent(${p.id}, '${p.display_name.replace(/'/g, "\\'")}')">刪除家長</button></tbody></table></div>`}
...
$("adminEventForm").onsubmit=async e=>{e.preventDefault();const id=$("editEventId").value,b={title:$("eventTitleInput").value,event_date:$("eventDateInput").value,location:$("eventLocationInput").value,meal_price:Number($("eventMealInput").value||0),event_type:$("eventTypeInput").value,response_deadline:$("eventDeadlineInput").value||null,player_ids:[...document.querySelectorAll("#eventPlayerChoices input:checked")].map(x=>Number(x.value))};if(id)b.status=$("eventStatusInput").value;try{await api(id?`/api/admin/events/${id}`:"/api/admin/events",{method:id?"PUT":"POST",body:JSON.stringify(b)});eventDialog.close();toast("活動已儲存");await Promise.all([loadEvents(),loadDashboard()])}catch(e){toast(e.message)}};
