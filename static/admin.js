...
async function loadEvents(){cache.events=await api("/api/admin/events");$("eventTable").innerHTML=`<div class="tableWrap"><table><thead><tr><th>日期</th><th>活動</th><th>類型</th><th>通知對象</th><th>回覆</th><th>出席</th><th>請假</th><th>操作</th></tr></thead><tbody>${cache.events.map(e=>`<tr><td>${e.event_date}</td><td><strong>${e.title}</strong><div>${e.location}</div><div class="muted">集合：${e.meet_time_tbd?"未定":(e.meet_time||"未定")}｜${e.match_count||0} 場</div></td><td><span class="tag">${e.event_type==="game"?"比賽":e.event_type==="practice"?"練球":"活動"}</span></td><td>${e.invited} 位球員</td><td>${e.replied}/${e.invited} <span class="tag ${Number(e.replied)<Number(e.invited)?"amber":"green"}">${Number(e.invited)-Number(e.replied)} 未回覆</span></td><td>${e.attend}</td><td>${e.leave}</td><td><div class="rowActions"><button onclick="viewEvent(${e.id})">統計 / 通知</button><button onclick="editEvent(${e.id})">編輯</button><button onclick="deleteEvent(${e.id}, '${e.title.replace(/'/g, "\\'")}')">刪除</button></div></td></tr>`).join("")}</tbody></table></div>`}
...
window.viewEvent=async id=>{try{currentEventId=id;const d=await api(`/api/admin/events/${id}`);$("eventDetailTitle").textContent=`${d.event.event_date}｜${d.event.title}`;const matchInfo=(d.matches||[]).map((m,i)=>`<span class="tag">第${i+1}場 ${m.game_time_tbd?"未定":(m.game_time||"未定")} vs ${m.opponent||"未定"}</span>`).join("");const replied=d.players.filter(x=>x.attendance_status).length,attend=d.players.filter(x=>x.attendance_status==="attend").length,leave=d.players.filter(x=>x.attendance_status==="leave").length,pending=d.players.length-replied;$("eventSummary").innerHTML=[["集合",d.event.meet_time_tbd?"未定":(d.event.meet_time||"未定")],["出席",attend],["請假",leave],["未回覆",pending]].map(x=>`<div class="stat"><div class="l">${x[0]}</div><div class="k">${x[1]}</div></div>`).join("");$("eventDetailBody").innerHTML=`<div class="panel"><strong>比賽場次</strong><div class="recipientList">${matchInfo||"<span class=\"tag amber\">尚未設定場次</span>"}</div></div><div class="tableWrap"><table><thead><tr><th>球員</th><th>組別</th><th>回覆</th><th>出席方式 / 備註</th><th>請假原因</th></tr></thead><tbody>${d.players.map(p=>`<tr><td><strong>${p.name}</strong></td><td>${p.team}</td><td>${p.attendance_status==="attend"&&d.event.event_type==="practice"&&p.practice_duration==="morning_leave"?'<span class="tag amber">下午出席</span>':p.attendance_status==="attend"&&d.event.event_type==="practice"&&p.practice_duration==="afternoon_leave"?'<span class="tag amber">上午出席</span>':p.attendance_status==="attend"&&d.event.event_type==="practice"&&p.practice_duration==="half"?'<span class="tag amber">半天（舊資料）</span>':p.attendance_status==="attend"?'<span class="tag green">出席</span>':p.attendance_status==="leave"?'<span class="tag red">請假</span>':p.attendance_status==="maybe"?'<span class="tag amber">未確定</span>':'<span class="tag amber">未回覆</span>'}</td><td>${p.attendance_status==="attend"?(d.event.event_type==="practice"?(p.practice_duration==="morning_leave"?"下午出席":p.practice_duration==="afternoon_leave"?"上午出席":p.practice_duration==="half"?"半天（舊資料）":"出席"):(p.attendance_note||"-")):"-"}</td><td>${p.leave_reason||"-"}</td></tr>`).join("")}</tbody></table></div>`;$("primaryOnly").checked=false;await Promise.all([loadTargets("all"),loadLogs()]);eventDetailDialog.showModal()}catch(e){toast(e.message)}};
